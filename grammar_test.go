...
func (s *GrammarSuite) TestCompileRenameColumn() {
	...
	setup("8.0.3")
	sql, err := s.grammar.CompileRenameColumn(mockSchema, mockBlueprint, &contractsdriver.Command{
		Column: mockColumn,
		From:   "before",
		To:     "after",
	})
	...
	setup("5.7.2")
	mockSchema.EXPECT().GetColumns("users").Return([]contractsdriver.Column{
		{
